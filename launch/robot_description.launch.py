...
def generate_launch_description():
    ...
    try:
        ...
        # 验证关键文件是否存在
        urdf_test = Path(landerpi_description_package_path) / 'urdf' / 'landerpi.xacro'
        rviz_test = Path(landerpi_description_package_path) / 'rviz' / 'view.rviz'
        if not (urdf_test.exists() and rviz_test.exists()):
            raise FileNotFoundError(
                f"Required files not found in {landerpi_description_package_path}. "
                "Check your package structure or run 'colcon build'."
            )
    ...
    robot_state_publisher_node = Node(
        ...
        # 注意：arguments=[urdf_path] 是多余的，已移除（robot_description 已提供）
        remappings=[
            ('/tf', 'tf'),
            ('/tf_static', 'tf_static')
        ]
    ...
    rviz_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(landerpi_description_package_path, 'launch', 'rviz.launch.py')
        ),
